...
import { retrieve, uploadFile, listFiles, deleteFile, downloadFile, libraryMeta, websiteOf, crawlWebsite, websiteStatus, deleteWebsite } from "./library.js";
...
async function handleChat(request, env, ctx, { wantEmail = false, isAdmin = false } = {}) {
  ...
  //     crawled pages for THIS question. "" if none, or if AI Search isn't set
  //     up — the bot answers from knowledge/ regardless.
  const found = await retrieve(env, CONFIG, project.id || CONFIG.defaultProject, last.content, { website: websiteOf(project) });
