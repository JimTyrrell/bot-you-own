...
// ---------------------------------------------------------------------------
export async function retrieve(env, config, bot, question, { website = null } = {}) {
  const none = { text: "", library: false, website: false };
  ...
  return {
    text: picked.map((c) => `<excerpt from="${c.from.replace(/"/g, "")}">\n${c.text}\n</excerpt>`).join("\n\n"),
    library: picked.some((c) => !c.web),
    website: picked.some((c) => c.web),
  };
}

// The crawler files each page under its URL (sometimes just the path, or a
// metadata field). Whatever it gives us, the excerpt is labelled with a full address.
